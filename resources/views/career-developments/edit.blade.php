      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
              <div class="p-6 bg-white border-b border-gray-200">
                  <form action="{{ route('career-developments.update', $careerDevelopment) }}" method="POST">
                      @csrf
                      @method('PUT')
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                              ...
                              <x-select id="employee_id" class="block mt-1 w-full" name="employee_id" required>
                                  ...
                                  @foreach($employees as $employee)
                                      <option value="{{ $employee->id }}" {{ old('employee_id', $careerDevelopment->employee_id) == $employee->id ? 'selected' : '' }}>
                          ...
                          <div>
                              ...
                              <x-select id="type" class="block mt-1 w-full" name="type" required>
                                  ...
                                  <option value="Promotion" {{ old('type', $careerDevelopment->type) == 'Promotion' ? 'selected' : '' }}>{{ __('Promotion') }}</option>
                                  <option value="Salary Increase" {{ old('type', $careerDevelopment->type) == 'Salary Increase' ? 'selected' : '' }}>{{ __('Salary Increase') }}</option>
                                  <option value="Training" {{ old('type', $careerDevelopment->type) == 'Training' ? 'selected' : '' }}>{{ __('Training') }}</option>
                                  <option value="Position Change" {{ old('type', $careerDevelopment->type) == 'Position Change' ? 'selected' : '' }}>{{ __('Position Change') }}</option>
                              </x-select>
                              @error('type')
                                  <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                              @enderror
                          ...
                          <div>
                              ...
                              <x-input id="previous_value" class="block mt-1 w-full" type="text" name="previous_value" :value="old('previous_value', $careerDevelopment->previous_value)" />
                          ...
                          <div>
                              ...
                              <x-input id="new_value" class="block mt-1 w-full" type="text" name="new_value" :value="old('new_value', $careerDevelopment->new_value)" />
                          ...
                          <div>
                              ...
                              <x-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', $careerDevelopment->date->format('Y-m-d'))" required />
                              @error('date')
                                  <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                              @enderror
                          </div>
                          ...
                          <div class="md:col-span-2">
                              ...
                              <x-textarea id="description" class="block mt-1 w-full" name="description">{{ old('description', $careerDevelopment->description) }}</x-textarea>
                              @error('description')
                                  <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                              @enderror
                          </div>
                      ...
                      <div class="flex items-center justify-end mt-4">
                          <a href="{{ route('career-developments.show', $careerDevelopment) }}" class="text-gray-600 hover:text-gray-900 mr-4">{{ __('Cancel') }}</a>
                          <x-button>
                              {{ __('Update Development Record') }}
                          </x-button>
                      </div>
                  </form>
              </div>
